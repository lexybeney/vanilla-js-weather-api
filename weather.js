...
async function getWeatherData(latitude, longitude) {
  ...
  getLocationFromCoords(latitude, longitude, weatherData);
}

// Use coords to get town from third-party API and update interface with this data
async function getLocationFromCoords(latitude, longitude, weatherData) {
  ...
  updateInterface(location.data[0].name, weatherData.data, latitude, longitude);
}
